...
#
# Modified by Chaolin Zhang, Feb 2017
#
...
sub next_structure {
   ...
   until( !defined $buffer ) {
       ...
       if (/^OBSLTE / && $all_headers) {
		$obslte = $self->_read_PDB_singlecontline("OBSLTE","12-75",\$buffer);
		$header{'obslte'} = $obslte;
       }

       # TITLE line(s)
       if (/^TITLE / && $all_headers) {
		$title = $self->_read_PDB_singlecontline("TITLE","11-80",\$buffer);
		$header{'title'} = $title;
       }

       # CAVEAT line(s)
       if (/^CAVEAT / && $all_headers) {
		$caveat = $self->_read_PDB_singlecontline("CAVEAT","12-79",\$buffer);
		$header{'caveat'} = $caveat;
       }

       # COMPND line(s)
       if (/^COMPND / && $all_headers) {
		$compnd = $self->_read_PDB_singlecontline("COMPND","11-80",\$buffer);
...
$self->debug("get COMPND $compnd\n");
	...
	if (/^SOURCE / && $all_headers) {
		$source = $self->_read_PDB_singlecontline("SOURCE","11-79",\$buffer);
		$header{'source'} = $source;
	}

	# KEYWDS line(s)
	if (/^KEYWDS / && $all_headers) {
		$keywds = $self->_read_PDB_singlecontline("KEYWDS","11-79",\$buffer);
		$header{'keywds'} = $keywds;
	}

	# EXPDTA line(s)
	if (/^EXPDTA / && $all_headers) {
		$expdta = $self->_read_PDB_singlecontline("EXPDTA","11-79",\$buffer);
		$header{'expdta'} = $expdta;
	}

	# AUTHOR line(s)
	if (/^AUTHOR / && $all_headers) {
		$author = $self->_read_PDB_singlecontline("AUTHOR","11-79",\$buffer);
	...
	if (/^SPRSDE / && $all_headers) {
		$sprsde = $self->_read_PDB_singlecontline("SPRSDE","12-75",\$buffer);
...
sub _read_PDB_singlecontline {
	...
	my $rol_size = $end - $begin + 1;
	
	if($begin == 11) {
		$unpack_string .= "a$rol_size";
	}
	elsif($begin == 12) { # one additional space
		$unpack_string .= "x1 a$rol_size";
	} else {
		die "begin must be 11 or 12\n";
	}
	$_ = $$buffer;
	while (defined( $_ ||= $self->_readline ) ) {
		if ( /^$record/ ) {
			...
			$rol =~s/\s+$/ /g; #strip extra trailing space
...
sub _read_PDB_coordinate_section {
	...
	#fixed by Chaolin Zhang: in some cases, chain name can have two letters
	my $atom_unpack =   "x6 a5 x1 a4 a1 a3 a2 a4 a1 x3 a8 a8 a8 a6 a6 x6 a4 a2 a2";
	my $anisou_unpack = "x6 a5 x1 a4 a1 a3 a2 a4 a1 x1 a7 a7 a7 a7 a7 a7 a4 a2 a2";
...
$self->debug("_read_PDB_coor: parsing model $model_num\n");
		...
		if (/^(ATOM |HETATM|SIGATM)/) {
			...
			if ($chainID ne $chain_name) { # possibly a new chain
				...
				$chainID=~s/^\s+//g; #remove possible leading spaces, fixed by Chaolin Zhang 02/12/2017
